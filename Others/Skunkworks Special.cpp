...
int main() {
    ...
    Test {
        ...
        ll tot_evens = BigMod(2LL , even, mod) - 1;
        ll tot_odds = 0;
        if (odd) {
            tot_odds = BigMod(2LL , odd - 1, mod) - 1;
        }
        ll ans = ((tot_evens + tot_odds) % mod + ((tot_evens * tot_odds) % mod)) % mod;
        ans += 1;
        ans %= mod;
        ll final = BigMod(2LL, n , mod) - ans;
        final %= mod;
        final += mod;
        final %= mod;
        cout << "Case " << tc << ": " << final << "\n";
